...
import hormoneTests from "../Components/Tests/Hormones";
...
function ResultViewPage() {
  ...
  const [hormoneResults, setHormoneResults] = useState([]);
    ...
    fetch(`${API_URL}/result/${id}`)
      ...
      .then(data => {
        if (data.success) {
          ...
          setHormoneResults(JSON.parse(r.hormones || "[]"));
  ...
  const header = (
  <div>
    <h3 style={{ textAlign: "center" }}>معمل مستشفى الخوجلابي</h3>
    <p style={{ textAlign: "center" }}>Sudan</p>
    <hr />
  ...
  {/* ================== الصفحة 3 ================== */}
  {(patient.tests.includes("Chemistry") || patient.tests.includes("Hormones") || patient.tests.includes("Serology")) && (
  <div className="print-section">
    {header}
    {patient.tests.includes("Chemistry") && (
      <Chemistry title="CHEMISTRY" tests={chemistryResults} readOnly />
    )}

    {patient.tests.includes("Hormones") && (
      <Chemistry title="HORMONES" presetTests={hormoneTests} tests={hormoneResults} readOnly />
    )}
